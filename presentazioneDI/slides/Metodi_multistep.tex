...
\begin{itemize}
    ...
    \item Poiché, nel nostro esempio, l'autovalore è a frequenza altissime, il passo risulta estremamente piccolo (\( h\lambda \ll 2 \))
\end{itemize}

        \begin{figure}
        \centering        \includegraphics[width=.65\linewidth]{bode_stiff_mag.png}
        \label{fig:my_label}
        \end{figure}
\end{frame}
\end{comment}
...
\section{BDF}\label{sec:sec5}   
    ...
    \begin{frame}{Costruiamo BDF}
        ...
        \begin{itemize}
            ...
            \begin{center}
            \begin{tabular}{c  c}
            \( f(t_n,y_n) = \dfrac{1}{\beta_0}\dfrac{\sum_{j=0}^{k}\alpha_j y_{n-j}}{h } \) &
